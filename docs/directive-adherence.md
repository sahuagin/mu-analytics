...
retroactively over ~/ai-sessions on the analytics host), and use that to test enforcement
...
Prototype: `scripts/adherence_probe.py` reads raw jsonl (no DuckDB/the analytics host) over the
**local subset** for fast iteration. Graduates into `features.py` (over the `ev`
view) for the real run on the analytics host.
...
none→doc shift needs the historic corpus on **the analytics host** that straddles these dates.
Local mtimes are also unreliable (`stat` returned bad values). So **none→doc is
bound to the analytics host**; locally we measure age-independent base rates instead.
...
Observation: cc frequently writes files via heredoc (`cat > f <<EOF`), runs code
...
3. H4-as-stated unsupported in normalized local data; revisit on the analytics host with power.

Caveats: small n (context bands n=23–32, size bands n=6–12) → heredoc 2.9× + the 40-150k
spike need the analytics host power; only exposure controlled (degradation.py's multivariate regression is
...
Sessions have WINDOWS, not one good/bad label (observed arc: good open → rework+frustration
arc → exhaustion-leave overnight → gap → recovery-probe next morning; good sessions also left
...
- gaps: 16; leave-hour mode **mid-morning** (not the overnight of canonical bad days); rework/steer
  before-gap 43%; **recovery-probe after-gap 0%**.
- **Conclusion:** the windowing MECHANISM works, but the phenomenon (degradation arcs, 4am
  leaves, recovery probes) **isn't in the local corpus — it's on the analytics host** (long day-sessions).
...
**the analytics host** via `features.py`/`degradation.py` + the windowed probe over `~/ai-sessions`.
...
**Results** (`scripts/context_disparity.py`, `ev` on the analytics host; per-turn ctx =
...
## Synthesis (rounds 7–13, powered on the analytics host)
...
companion compliance-enforcement study — authoritative base rates it cites
over its independent grep numbers.
...
- **mu=1,398 cross-validates** the peer's independent grep count (1,397).
...
Deliverables (2) + (3) for the companion compliance-enforcement study — the labeled sets it needs to validate its
...
(internal postmortem notes, host-local). 6 sessions, **all present in ev**:

| date | behavior (peer's classes) | session_ref |
|---|---|---|
| 2026-06-05 | relitigation | ✓ in ev (cc + mu) |
| 2026-06-08 | map_as_terrain (premise-misframe) | ✓ in ev |
| 2026-06-12 | defend-on-correction / miscalibration | ✓ in ev |
| 2026-06-17 | dismissiveness | ✓ in ev |
| 2026-06-21 | scope_overreach | ✓ in ev |
...
positives. Written to a host-local TSV.
...
- Local subset only (bench-heavy on cc); **not the real baseline** — that's the
  full ~/ai-sessions corpus on the analytics host.
...
5. Run the whole thing on the analytics host over the real corpus.
