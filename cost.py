...
COLS = ("SELECT provider, model, exit_reason, outcome_class, "
...
def load_sink(label: str, db: str):
    ...
    try:
        ...
        rows = [dict(r) | {"rate_key": rate_key(r["model"]),
...
frames = [f for f in (load_sink("mu", PATHS["mu_sink_db"]),
                      load_sink("cc", PATHS["cc_sink_db"])) if f is not None]
if not frames:
...
priced = df.filter(pl.col("in_rate").is_not_null()).with_columns(
    (
        (pl.col("input_tok") * pl.col("in_rate")
         ...
         + pl.col("output_tok") * pl.col("out_rate")) / 1_000_000
    ).alias("cost_usd")
)

print("=== cost by KIND  (billed = real $ paid; subscription = API-equiv, NOT paid; free = $0) ===")
print(priced.group_by("cost_kind").agg(
    pl.len().alias("sessions"), pl.col("cost_usd").sum().round(2).alias("cost_usd")
).sort("cost_usd", descending=True))
print(f"   (+ {len(df.filter(pl.col('cost_kind') == 'free'))} free sessions at $0)\n")

print("=== cost by fleet ===")
print(priced.group_by("fleet").agg(
    pl.len().alias("sessions"), pl.col("cost_usd").sum().round(2).alias("cost_usd")
).sort("cost_usd", descending=True))

print("=== cost by fleet + model ===")
print(priced.group_by("fleet", "model").agg(
    pl.len().alias("sessions"), pl.col("cost_usd").sum().round(2).alias("cost_usd")
).sort("cost_usd", descending=True))

print(f"=== total: ${priced['cost_usd'].sum():,.2f} across {len(priced)} priced sessions ===")
if len(unpriced):
    print(f"!! {len(unpriced)} unpriced session(s) — models not in [rates]:",
...
manual = (top["input_tok"] * ir
          + top["cache_write"] * ir * MULT["write_5m"]
          + top["cache_read"] * ir * MULT["read"]
          + top["output_tok"] * orr) / 1_000_000
print("\n=== hand-check (most expensive session) ===")
print(f"model={top['model']}  input={top['input_tok']}  output={top['output_tok']}  "
      f"cache_read={top['cache_read']}  cache_write={top['cache_write']}")
print(f"  ({top['input_tok']}*{ir} + {top['cache_write']}*{ir}*{MULT['write_5m']} "
      f"+ {top['cache_read']}*{ir}*{MULT['read']} + {top['output_tok']}*{orr}) / 1e6")
print(f"  manual = ${manual:,.4f}   polars = ${top['cost_usd']:,.4f}   "
