...
on the .172 corpus. Markers verbatim from mu-analytics scans.py.
"""
import glob
import json
import os
...
NEG = re.compile("|".join([
    r"relitigat", r"whose money", r"why are (we|you)", r"that'?s not what", r"\bno\.\s*$",
    r"\bstop\b", r"\bwtf\b", r"wrong again", r"\bi said\b", r"you keep", r"hallucinat",
    r"not fun", r"\bbroken\b", r"don'?t do", r"\bagain\?", r"i didn'?t ask",
    r"burn(ing)? (money|tokens)", r"please don'?t", r"do not commit", r"stop reinforcing",
    r"don'?t feel like going (through|over)", r"we'?ve already done (it|this)",
    r"look in your memory", r"AI.?spla[in]", r"why restate", r"\bto be superior\b",
    r"i (already|just) (verified|gave|told|ran|showed)", r"run it all again",
...
def main():
    ...
    for f in cc:
        ...
        rows.append({
            ...
            "viol": V.violations(t), "mx": max(ctx) if ctx else 0,
    ...
    def band(rows, key, bands):
        for lo, hi, lab in bands:
            ...
            if b:
                ...
                print(f"  {lab:9} n={len(b):>3}  presence={pres:>3}%   rate={1000*rate(b):>5.1f}/1k")
    ...
    for p in ["heredoc", "code_in_heredoc", "shell_file_write", "large_bash", "edit_loop"]:
        ...
        print(f"  {p:18} rate|with={1000*rw:>5.1f}/1k  rate|without={1000*rwo:>5.1f}/1k  lift={lift:>4.1f}x")
