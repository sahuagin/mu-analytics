...
view, no remote host) so it runs over the local subset for fast iteration. The real
run graduates these signals into features.py (over the unified `ev` view) and
executes on the analytics host over ~/ai-sessions. See docs/directive-adherence.md.
...
def compactions(ctxs, drop_frac=0.30, drop_abs=20000):
    ...
    return sum(
        1
        for a, b in zip(ctxs, ctxs[1:], strict=False)
        if a > 0 and (a - b) >= max(drop_abs, drop_frac * a)
    )
...
def summarize(name, files, traj):
    ...
    print(
        f"  initial ctx : p10={q(firsts, 0.1):>7,}  med={int(st.median(firsts)):>7,}  p90={q(firsts, 0.9):>9,}"
    )
    print(
        f"  max ctx     : med={int(st.median(maxes)):>7,}  p90={q(maxes, 0.9):>9,}  max={max(maxes):>10,}"
    )
