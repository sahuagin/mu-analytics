...
Sessions aren't uniformly good/bad — they have WINDOWS (observed 2026-06-22):
a good opening, then a degradation arc (rising rework demands + frustration), an
exhaustion-leave overnight, a long gap, and a recovery-probe on return the next morning.
...
sessions live on the analytics host — so this is the MECHANISM + a weak local read. Markers are
heuristic (FP-prone); the real run is the analytics host.
"""
...
REWORK = re.compile(
    ...
    r"making more work|relitigat|stop reinforcing|why are (we|you)",
    re.I,
)
STEER = re.compile(
    r"\bstop\b|^\s*no[.,]?\s|\bdon'?t\b|that'?s not|\bwrong\b|not quite|^actually", re.I
)
POS = re.compile(
    r"thank|great work|good (work|job|call)|\bperfect\b|\bexcellent\b|nailed it|"
    r"looks good|\blgtm\b|that work|much better",
    re.I,
)
PROBE = re.compile(
    r"are you (still )?(there|here|working)|do you (still )?remember|"
    r"where (were|are) we|what (were|are) we|pick up where|catch (me )?up|"
    r"did you (finish|do|get|manage)|still (working|going|there|with me)|recover",
    re.I,
)
...
def main():
    ...
    def halves(rx):
        ...
        sh = [rate(s[len(s) // 2 :], rx) for s in sess]
        return st.median(f), st.median(sh), sum(1 for a, b in zip(f, sh, strict=False) if b > a)

    for lab, rx in [("REWORK", REWORK), ("STEER", STEER), ("POS", POS)]:
        mf, ml, rose = halves(rx)
        print(
            f"  {lab:7} 1st-half median={mf:.3f}  2nd-half median={ml:.3f}  rose in {rose}/{len(sess)} sessions"
        )
    ...
    if ngaps:
        ...
        print(
            f"  rework/steer in 2 msgs BEFORE gap: {pre_rework}/{ngaps} ({100 * pre_rework // ngaps}%)"
        )
        print(
            f"  recovery-probe in 1st msg AFTER gap: {post_probe}/{ngaps} ({100 * post_probe // ngaps}%)"
        )
