...
mu native) — one schema, no hand-parse. Run on the deployed host:
...
def report(name, rows):
    ...
    print(f"  initial ctx : med={int(st.median(init)):>8,}  p90={q(init, 0.9):>9,}")
    print(f"  max ctx     : med={int(st.median(mx)):>8,}  p90={q(mx, 0.9):>9,}  max={max(mx):>10,}")
